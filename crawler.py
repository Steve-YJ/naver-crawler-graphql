...
def upload_to_gcs(data, bucket_name, destination_file_name, key_path):
    ...
    # df = df.drop_duplicates(inplace=True) # 중복 제거
    df.drop_duplicates(subset=['name'], inplace=True) # 'name' 칼럼에 대해서만 중복을 제거하도록 변경

    # DataFrame을 Parquet 형식으로 변환
    buffer = BytesIO()
    df.to_parquet(buffer, index=False)
    buffer.seek(0)  # 버퍼의 시작 위치로 포인터 이동
...
def run_crawler(url, headers, query, key_path, bucket_name, file_name, file_prefix):
    """크롤러 실행 함수."""
    current_page = 0
    ...
    try:
        while True:
            if current_page == 0:
                pagenation = 1 # 초기 값 설정
                print(f"pagenation: {pagenation}")
            else:
                pagenation = current_page * restaurant_per_page # e.g. 70 >> 140 >> ...
                print(f"pagenation: {pagenation}")
            data = {
                ...
                "variables": {
                    ...
                    "restaurantListInput": {
                        ...
                        "x": "127.098619",
                        "y": "37.389844",
                        "start": pagenation,            # 초기값 1, 70 간격으로 이동
                        "display": restaurant_per_page, # 페이지 노출 범위. 기본값은 70
                        "isPcmap": True
                    },
            ...
            if response and response.json()['data']['restaurants']['items']:
                ...
                upload_to_gcs(items, bucket_name, file_name, key_path)
        ...
        # 파티션내에 전체 데이터 저장
        file_name = f"{file_prefix}/{date_path}/ROAD_{destination_file_name}" # bigquery에서 읽어올 때 prefix로 ROAD_를 찾아옴. e.g. ROAD_*.parquet
        upload_to_gcs(df, bucket_name, file_name, key_path)
...
if __name__ == "__main__":
    ...
    """환경 변수에서 검색어 가져오기"""
    input_query = os.getenv('INPUT_QUERY', '강남역 맛집')
    file_prefix = os.getenv('FILE_PREFIX', 'gangnam_gourmet')
    destination_file_name = os.getenv('DESTINATION_FILE_NAME', 'gangnam_gourmet.parquet')
    ...
    key_path = "./key.json"
    bucket_name = "naver-crawler-data-lake"
    # destination_file_name = "gangnam_gourmet.parquet"

    run_crawler(url, headers, input_query, key_path, bucket_name, destination_file_name, file_prefix)
